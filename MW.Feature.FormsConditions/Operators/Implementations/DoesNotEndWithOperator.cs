...
{
    public class DoesNotEndWithOperator : StringOperatorBase, IOperator
    {
        public Guid Id => Constants.Items.Operator.DoesNotEndWith;

        protected override bool IsMatch(string fieldValue, string operatorValue)
        {
            return !fieldValue.EndsWith(operatorValue);
        }
    }
}